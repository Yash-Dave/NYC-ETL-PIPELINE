...
Architecture
NYC DOHMH SODA API (400k rows)
        │
        ▼
Python ingestion script (Lambda or cron)
        │  paginated 50k/page · retry · structured logs
        ▼
S3 Bronze (.ndjson) ──► RDS PostgreSQL bronze.restaurant_inspections
                                    │
                          dbt · stg_inspections (silver)
                          cast · dedup · filter sentinel dates
                                    │
                    ┌───────────────┴───────────────┐
                    ▼                               ▼
         fct_inspections              agg_borough_kpis
         (per-inspection)             (monthly KPIs per borough)
                    │
        Great Expectations (17 rules)
                    │ JSON events
        CloudWatch Logs ──► Metric Filter ──► Alarm ──► SNS email
                    │
        Power BI dashboard (compliance KPIs, borough drill-down)
Stack
Layer	Tech
...
4. Run the pipeline
bash
python ingest/fetch_to_s3_postgres.py   # ~20 mins for 400k rows

cd dbt_project && dbt run && dbt test

python ge/create_suite.py

ALARM_EMAIL=you@email.com bash cloudwatch/setup_alarms.sh   # run once
5. GitHub Actions
